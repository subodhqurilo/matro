  ...
  useEffect(() => {
    ...
    const fetchNewlyMatched = async () => {
      try {
        ...
        const response = await fetch('https://bxcfrrl4-3000.inc1.devtunnels.ms/api/profile/newly-user', {
          method: 'GET',
          headers: {
        ...
        setNewlyMatched(mappedProfiles);
      } catch (err) {
        console.error('Failed to fetch newly matched users:', err);
        setError(err instanceof Error ? err.message : 'Failed to fetch newly matched users');
        toast.error('Failed to load newly matched users. Please try again.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-4">
      {isLoading ? (
        ...
        <div className="text-center py-8 text-gray-600">No newly matched users found.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {newlyMatched.map((profile) => (
