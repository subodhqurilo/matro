...
const API_BASE_URL = "http://localhost:3000/api/like/iShortlist"
...
export default function IShortlisted() {
  ...
  const fetchIShortlistedProfiles = async () => {
    try {
      setLoading(true)
      setError(null)
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.message || "Failed to fetch I Shortlisted profiles")
      }
  ...
  useEffect(() => {
    fetchIShortlistedProfiles()
  }, [])

  // Optional: Remove profile dynamically
  const removeProfile = (id: string) => {
    setProfiles(prev => prev.filter(profile => profile.id !== id))
  }

  if (loading) return <p className="text-center">Loading I Shortlisted profiles...</p>
